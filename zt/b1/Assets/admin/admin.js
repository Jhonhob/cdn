...
var user_distribution_settings = jQuery('#b2_distribution_ratio').find(':selected').val();
if(user_distribution_settings == 1){
	jQuery('#distribution').show()
	
}else{
	jQuery('#distribution').hide()
}
jQuery('#b2_distribution_ratio').on('change', function() {
	var val = jQuery(this).find(':selected').val();
	...
	user_distribution_settings = val
})
...
if(b2chat == 'none'){
	...
	jQuery('#aside_chat_weixin_d').hide();
}else if(b2chat == 'qq'){
	...
	jQuery('#aside_chat_weixin_d').hide();
}else if(b2chat == 'weixin'){
	jQuery('#aside_chat_qq_d').hide();
	jQuery('#aside_chat_crisp_d').hide();
	jQuery('#aside_chat_dmsg_d').hide();
	jQuery('#aside_chat_weixin_d').show();
}else{
	jQuery('#aside_chat_qq_d').hide();
	jQuery('#aside_chat_crisp_d').hide();
	jQuery('#aside_chat_dmsg_d').hide();
...
const fliterA = new Vue({
	...
	mounted(){
		...
		this.start = params.get('start'); // "1"
		this.end = params.get('end'); // "2" 
	},
	methods: {
		fliter(){
			...
			url.searchParams.set('start', new Date(this.start).Format("yyyy-MM-dd"));
			url.searchParams.set('end', new Date(this.end).Format("yyyy-MM-dd"));
	...
	const userTop = new Vue({
		...
		methods:{
			getUserList(){
				this.locked = true
				jQuery.ajax({
					url: cmb2_l10.ajaxurl+'?action=user_list',
					type: 'POST',
					success: (data)=>{
			...
			getCreditList(){
				this.locked = true
				jQuery.ajax({
					url: cmb2_l10.ajaxurl+'?action=user_list_credit',
					type: 'POST',
					data:{
						paged:this.cpaged
					},
					success: (data)=>{
						this.list.credit.list = JSON.parse(data)
						this.locked = false
					}
				});
			},
			getMoneyList(){
				this.locked = true
				jQuery.ajax({
					url: cmb2_l10.ajaxurl+'?action=user_list_money',
					type: 'POST',
					data:{
						paged:this.mpaged
					},
					success: (data)=>{
						this.list.money.list = JSON.parse(data)
						this.locked = false
					}
				});
			},
