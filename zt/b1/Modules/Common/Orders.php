...
class Orders{
    ...
    public static function build_order_action($data,$_order_id = ''){
        ...
        //检查限购
        $limit = apply_filters('b2_shop_limit',$data['post_id'],$user_id,$data['order_count']);
        if(!$limit['can_buy']){
            if($limit['buy_count'] + $data['order_count'] > $limit['limit']){
                return array('error'=>__('超出限购数量，请减少数量后重新下单','b2'));
            }

            return array('error'=>__('限购商品，无法再次购买','b2'));
        }
    ...
    public static function get_my_orders($_user_id,$paged,$state = 'w'){
        ...
        $order = $wpdb->get_results(
            $wpdb->prepare("
                ...
                ",
                $user_id,'coupon','g',$offset,$number
            )
        ,ARRAY_A);
