...
class Main{
    ...
    public function setup_frontend_scripts(){
        ...
        //加载字体
        wp_enqueue_style( 'b2-fonts', '//at.alicdn.com/t/c/font_2579934_o609iysqgdb.css' , array() , B2_VERSION , 'all');
        ...
        $shop_row = b2_get_option('shop_main','shop_type_count_row');

        if($shop_row){
            $css .= '.shop-normal-item{width:'.(100/$shop_row).'%!important}';
        }

        $collection_row_number = b2_get_option('template_collection','collection_row_number');
        $collection_row_number = $collection_row_number ? $collection_row_number : 2;
        $collection_row_number = 100/$collection_row_number.'%';
