...
class RestApi{
    ...
    public function b2_rest_regeister(){
        ...
        //获取商品分类
        register_rest_route('b2/v1','/getShopList',array(
            'methods'=>'post',
            'callback'=>array(__CLASS__,'getShopList'),
            'permission_callback' => '__return_true'
        ));
        ...
        if(isset($res['error'])){
            ...
            foreach ($res['data'] as $k => $v) {
                $html .= '<li>
                    ...
                    <a href="'.$v['link'].'" target="_blank"><img src="'.b2_get_thumb(array('thumb'=>$v['thumb'],'type'=>'fill','width'=>'120','height'=>'74')).'" class="avatar b2-radius" /></a>
                </div>' : '').'
                    <div class="following-info">
    ...
    public static function getShopList($request){
        $res = Shop::get_shop_list($request);
