...
class Post{
    ...
    public function publish_post($new_status, $old_status, $post){
        ...
        $open = b2_get_option('normal_mail_message','msg_pending_admin');
        $open =  $open === '' ? true : $open;

        $open_author = b2_get_option('normal_mail_message','msg_pending_author');
        $open_author =  $open_author === '' ? true : $open_author;
        ...
        switch ($post_type) {
            ...
            case 'post':
                ...
                if($new_status == 'pending' && $open){
                    $user_data = get_userdata($user_id);
                    Msg::send_email_to_user(
                        [
                            'email' => $admin_email,
                            'object' => __('待审文章请审核','b2'),
                            'content' => '<p>'.sprintf(__('%s提交了待审文章%s','b2'),
                            '「'.$user_data->display_name.'」',
                            '<a href="'.admin_url('wp-admin/post.php?post='.$post->ID.'&action=edit').'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
                        ]
                    );
                }
                ...
                if($open_author){
                    Msg::send_email_to_user(
                        [
                            'user_id' => $user_id,
                            'object' => __('文章审核成功','b2'),
                            'content' => '<p>'.sprintf(__('您提交的文章审核成功：%s','b2'),
            ...
            case 'newsflashes':
                ...
                if($new_status == 'pending' && $open){
                    ...
                    Msg::send_email_to_user(
                        [
                            'email' => $admin_email,
                            'object' => sprintf(__('待审%s请审核','b2'),$newsflashes_name),
                            'content' => '<p>'.sprintf(__('%s提交了待审%s','b2'),
                            '「'.$user_data->display_name.'」',
                            $newsflashes_name.'：<a href="'.admin_url('wp-admin/post.php?post='.$post->ID.'&action=edit').'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
                        ]
                    );
                }
                ...
                if($open_author){
                    Msg::send_email_to_user(
                        [
                            'user_id' => $user_id,
                            'object' => sprintf(__('%s审核成功','b2'),$newsflashes_name),
                            'content' => '<p>'.sprintf(__('您提交的%s审核成功：%s','b2'),
                            $newsflashes_name,
                            '<a href="'.get_permalink($post->ID).'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
                        ]
            ...
            case 'circle':
                ...
                if($new_status == 'pending' && $open){
                    $circle_data = CircleRelate::get_data(array(
                        'circle_id'=>$circle_id,
                        'circle_role'=>'admin'
                    ));
                    if(isset($circle_data[0]['user_id'])){
                        $user_data = get_userdata($user_id);
                        Msg::send_email_to_user(
                            [
                                'user_id' => $circle_data[0]['user_id'],
                                'object' => sprintf(__('待审%s帖子请审核','b2'),$circle_name),
                                'content' => '<p>'.sprintf(__('%s提交了待审%s','b2'),
                                '「'.$user_data->display_name.'」',
                                $circle_name.__('帖子','b2').'：<a href="'.get_term_link($circle_id).'">'.get_the_title($post->ID).'</a>'
                                ).'</p>'
                            ]
                        );
                    }
                ...
                if($open_author){
                    Msg::send_email_to_user(
                        [
                            'user_id' => $user_id,
                            'object' => sprintf(__('%s帖子审核成功','b2'),$circle_name),
                            'content' => '<p>'.sprintf(__('您提交的%s帖子审核成功：%s','b2'),
                            $circle_name,
                            '<a href="'.get_permalink($post->ID).'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
                        ]
            ...
            case 'infomation':
                ...
                if($new_status == 'pending' && $open){
                    ...
                    Msg::send_email_to_user(
                        [
                            'email' => $admin_email,
                            'object' => sprintf(__('待审%s请审核','b2'),$infomation_name),
                            'content' => '<p>'.sprintf(__('%s提交了待审%s','b2'),
                            '「'.$user_data->display_name.'」',
                            $infomation_name.'：<a href="'.$term_link = get_permalink($post->ID).'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
                        ]
                    );
                }
                ...
                if($open_author){
                    Msg::send_email_to_user(
                        [
                            'user_id' => $user_id,
                            'object' => sprintf(__('%s审核成功','b2'),$infomation_name),
                            'content' => '<p>'.sprintf(__('您提交的%s审核成功：%s','b2'),
                            $infomation_name,
                            '<a href="'.get_permalink($post->ID).'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
            ...
            case 'ask':
                ...
                if($new_status == 'pending' && $open){
                    ...
                    Msg::send_email_to_user(
                        [
                            'email' => $admin_email,
                            'object' => sprintf(__('待审%s请审核','b2'),$ask_name),
                            'content' => '<p>'.sprintf(__('%s提交了待审%s','b2'),
                            '「'.$user_data->display_name.'」',
                            $ask_name.'：<a href="'.$term_link = get_permalink($post->ID).'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
                        ]
                    );
                }
                ...
                if($open_author){
                    Msg::send_email_to_user(
                        [
                            'user_id' => $user_id,
                            'object' => sprintf(__('%s审核成功','b2'),$ask_name),
                            'content' => '<p>'.sprintf(__('您提交的%s审核成功：%s','b2'),
                            $ask_name,
                            '<a href="'.get_permalink($post->ID).'">'.get_the_title($post->ID).'</a>'
                            ).'</p>'
    ...
    public static function insert_post($data){
        ...
        //检查下载
        $data = apply_filters('b2_download_data_clean',$data);

        $download_str = '';
        if(isset($data['download']) && is_array($data['download'])){
            foreach($data['download'] as $k=>$v){
                $download_str .= $data['download'][$k]['rights_save'].$data['download'][$k]['url_save'];
            }
        }

        $censor = apply_filters('b2_text_censor', $data['title'].$data['content'].$data['excerpt'].implode(',',$data['tags']).$download_str);
        ...
        if($post_id){
            ...
            if(isset($data['from']['url']) && $data['from']['url']){
                ...
                update_post_meta($post_id,'b2_post_from_url',esc_url(b2_remove_kh($data['from']['url'])));

            }

            if(isset($data['from']['name']) && $data['from']['name']){
                update_post_meta($post_id,'b2_post_from_name',b2_remove_kh($data['from']['name']));
            }

            //权限设置
            if(isset($data['role'])){
                update_post_meta($post_id,'b2_post_reading_role',b2_remove_kh($data['role']['key']));
                if(isset($data['role']['money']) && $data['role']['money']){
                    ...
                    update_post_meta($post_id,'b2_post_money',b2_remove_kh($data['role']['money']));
                }
            ...
            do_action('b2_post_insert_success',$post_id,$user_id,$data);
    ...
    public static function user_can_edit($post_id,$user_id){
        ...
        $time = Circle::user_can_delete_post($post_id,$user_id);
