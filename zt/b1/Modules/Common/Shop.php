...
class Shop{
    ...
    public static function get_shop_item_data($post_id,$user_id,$return = array(),$index = ''){
        ...
        $limit = apply_filters('b2_shop_limit',$post_id,$user_id);

        $can_buy = self::user_can_buy($post_id,$user_id,$type,$limit);
        ...
        if($type == 'normal' && $shop_multi == 1){
            $multi = self::get_multi_data($post_id,$user_id,$limit);
        }
    ...
    public static function get_multi_data($post_id,$user_id,$limit = []){
        ...
        if(isset($limit['can_buy'])){
            if(!$limit['can_buy']){
                $limit_text = __('限购商品，无法再次购买','b2');
            }
        }
            ...
            foreach ($data['attrs'] as $k => $v) {
                ...
                if($stock['allow'] === false){
                    ...
                    $can_buy = array(
                        'roles'=>$_can_buy['roles'],
                        'allow'=>true,
                        'text'=>$limit_text ? $limit_text : __('立刻购买','b2')
                    );
                }
    ...
    //用户的购买权限
    public static function user_can_buy($post_id,$user_id,$type,$limit = []){
        ...
        switch ($type) {
            ...
            case 'lottery':
                ...
                return array(
                    ...
                    'lvs'=>$_lvs,
                    'allow'=>true,
                    'text'=>$limit_text ? $limit_text : __('抽奖','b2')
                );
            ...
            case 'exchange':
                if($stock['allow'] === false){
                    ...
                    return array(
                        ...
                        'lvs'=>$_lvs,
                        'allow'=>true,
                        'text'=>$limit_text ? $limit_text : __('兑换','b2')
                    );
                }
    ...
    public static function get_shop_list($data){

        $user_id = b2_get_current_user_id();

        $paged = isset($data['paged']) && $data['paged'] ? (int)$data['paged'] : 1;
        $count = isset($data['count']) && $data['count'] ? (int)$data['count'] : 10;

        $w = isset($data['w']) && $data['w'] ? $data['w'] : 200;
        $h = isset($data['h']) && $data['h'] ? $data['h'] : 200;
        $ratio = isset($data['ratio']) && $data['ratio'] ? $data['ratio'] : 1;
        ...
        $args = apply_filters('b2_shop_module_args',$args,$data);
        ...
        wp_reset_postdata();
        $list .= \B2\Modules\Templates\Modules\Products::ids_to_html($ids,$w,$h,$ratio,$open);
        return apply_filters( 'b2_shop_module_lsit', $list,$data,$args);
    }
