...
//游客默认可以购买下载
// function b2_guest_can_buy($metadata, $object_id, $meta_key, $single) {
//     if($meta_key=='b2_down_guest_buy') {
//        return 1;
//     }
// }
// add_filter('get_post_metadata', 'b2_guest_can_buy', 99, 4);
...
function b2_text($data = array()){
	...
	return array(
		...
		'global'=>array(
			...
			'clear_history'=>__('确认要清除浏览记录吗？','b2'),
...
function b2_weixin_message_templates(){
	return apply_filters('b2_weixin_message_templates', [
		'pay_success'=>[
			'name'=>'订单支付成功提醒',
			'id'=>'KiYMCfEeFBhoKAbgirgciFOPgtxyGP3Y6bLmEFKShnc',
			'temp_id'=>'',
			'template_id_short'=>'46727',
			'keyword_name_list'=>['商品名称','订单号','支付金额','下单时间']
		],
		'new_order'=>[
			'name'=>'新订单通知',
			'id'=>'EEFQQ0c_YZF-JEgMqLs8ltgia7wYxL6XpFl0ANcfEcA',
			'temp_id'=>'',
			'template_id_short'=>'48379',
			'keyword_name_list'=>['产品名称','订单编号','订单金额','订单状态']
		],
		'order_ship'=>[
			'name'=>'订单发货通知',
			'id'=>'OibfJr9lcsH-DkbSwXVa-z1recuqVvLVDXbHUnciS40',
			'temp_id'=>'',
			'template_id_short'=>'48233',
			'keyword_name_list'=>['商品名称','物流公司','物流单号','订货单号']
		],
		'check_success'=>[
			'name'=>'审核通过通知',
			'id'=>'	K_hXInp8L7EVfgi-7ZzMkxBOzadwnWQFGJ_KNodNTZ4',
			'temp_id'=>'',
			'template_id_short'=>'48172',
			'keyword_name_list'=>['流程名称','审批时间','审批结果']
		],
	]);
}
	...
	if($juhe){
		...
		if($types && !empty($types)){
			foreach ($types as $key => $value) {
				...
				$normal[($value == 'qq' ? 'juheqq' : ($value == 'baidu' ? 'juhebaidu' : $value))] = array(
					'name'=>ucfirst($name),
					'icon'=>$icon,
					'color'=>$color,
...
// add_action('b2_aside_bar','b2_aside_bar');
// function b2_aside_bar(){
// 	echo '<div class="gdd-quick-link-buy-vip" style="margin-top:10px">
// 			<a target="_blank" href="https://baidu.com?aff=31077">
// 				<div class="gdd-quick-link-buy-vip__hover-block">
// 					<img src="'.B2_THEME_URI.'/Assets/fontend/images/gift.svg'.'" style="width: 20px;height: 20px;"><p>自定义按钮</p>
// 				</div>
// 			</a>
// 	</div>';
// }

// function b2_remove_category( $string, $type )  {
...
// 	return $string;  
// }     
// add_filter( 'user_trailingslashit', 'b2_remove_category', 100, 2);
