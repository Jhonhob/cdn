    ...
    public function register_taxonomy_metabox(){
        ...
        if(!$is_circle_default){
            $cmb_circle->add_field( array( 
                ...
                'default'=>'free',
                'desc'=>__('免费圈子任何人都可以申请加入；付费圈子需要缴纳一定的金额才能入圈；专属圈子只允许特定等级的用户入圈，具体策略请在下面设置。','b2')
            ) );
            ...
            $cmb_circle->add_field( array( 
                'name'     => sprintf(__( '允许查看前几篇帖子', 'b2' ),$circle_name), 
                'id'       => 'b2_circle_can_read', 
                'type' => 'text',
                'default'=>0,
                'desc'=>sprintf(__('如果此%s内帖子只对%s开放，您可以允许未加入的人查看前几篇帖子，如果不填写或者填写0，则未加入的人可查看的帖子数为0。','b2'),$circle_name,$circle_member_name),
                'after_row'=>'</div></div>',
            ) );
