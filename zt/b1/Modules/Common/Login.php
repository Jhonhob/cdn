...
class Login{
    ...
    public static function get_user_info($ref){
        ...
        $role = array(
            ...
            'infomation'=>User::check_user_role($current_user_id,'infomation'),
            'ask'=>User::check_user_role($current_user_id,'ask'),
            'link'=>User::check_user_role($current_user_id,'link'),
    ...
    public static function regeister_action($data,$check_type,$check_invitation){
        ...
        //邀请码使用
        if($check_invitation){
            Invitation::useInv($user_id,$check_invitation['id'],$check_invitation['invitation_owner']);
    ...
    public static function check_email($email){
        ...
        //获取白名单
        $white_list = b2_get_option('normal_login','email_whitelist');

        //获取黑名单
        $black_list = b2_get_option('normal_login','email_blacklist');

        //去掉邮箱前后的空白
        $black_list = trim($black_list, " \t\n\r\0\x0B\xC2\xA0");
        $white_list = trim($white_list, " \t\n\r\0\x0B\xC2\xA0");

        if(!$black_list && !$white_list) return $email;

        $black_list = explode(PHP_EOL, $black_list );
        ...
        //如果邮箱在黑名单中
        if(in_array($_email,$black_list)){
            return array('error'=>sprintf(__('不能使用%s的邮箱在本站注册','b2'),$_email));
        }
    ...
    public static function check_username($username){
        ...
        if(is_email($username)){
            $check = self::check_email($username);
            if(isset($check['error'])) return $check;
        }
