...
class Circle{
    ...
    public static function get_circle_top_cats(){
        ...
        if(user_can($current_user,'manage_options')) $circle_square = 1;
    ...
    public static function get_topic_list($data){
        ...
        $read_index = (int)get_term_meta($circle_id,'b2_circle_can_read',true);
        ...
        if ( $topic_query->have_posts()) {
            ...
            while ( $topic_query->have_posts() ) {
                ...
                if($paged == 1 && $i < $read_index + 1){
                    $can_read = true;
                }else{
                    $can_read = false;
                }

                $topic_data = self::get_data_by_topic_id($topic_id,$user_id,$topic_query,$can_read);
    ...
    //通过话题ID获取话题信息
    public static function get_data_by_topic_id($topic_id,$user_id = 0,$topic_query = null,$can_read = false){
        ...
        if($can_read == true && !$allow_read['allow']){
            $allow_read['allow'] = true;
        }
        ...
        $favorites = Post::get_post_favorites($current_user_id,$topic_id);
    ...
    public static function get_circles_list($data){
        ...
        if($data['type'] === 'hot'){
            ...
            if($data['type'] === 'my'){
                $ids[] = get_option('b2_circle_default');
            }
