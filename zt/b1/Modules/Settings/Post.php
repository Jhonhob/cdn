...
class Post{
    ...
    */
    ...
    public function post_side_settings(){
        ...
        $distribution_meta->add_field(array(
            'name' => __('限购数量','b2'),
            'id'   => 'b2_shop_limit_num',
            'type' => 'text',
            'desc'=>__('如果设置为0或留空，则不限限制购买，否则请直接填写数字，比如每人限购10件，则填写10。','b2'),
        ));

        //限购时间
        $distribution_meta->add_field(array(
            'name' => __('限购时间（天）','b2'),
            'id'   => 'b2_shop_limit_time',
            'type' => 'text',
            'desc'=>__('如果设置为0或留空，则限购无时间限制。如果限购活动限制在2天内，则直接填2，第三天将停止限购，以此类推。到期后会重新计算。','b2'),
        ));
