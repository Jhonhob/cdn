                    ...
                    <div class="post-pre">
                        <div class="post-pre-next-in">
                            <div class="post-pre-next-info b2-pd b2-radius box">
                                <div class="next-jt"><a href="'.$data['pre']['link'].'">'.b2_get_icon('b2-arrow-left-s-line').'</a></div>
                    ...
                    <div class="post-next">
                        <div class="post-pre-next-in">
                            <div class="post-pre-next-info b2-pd b2-radius box">
                                ...
                                <p>'.$data['next']['date'].'</p>
                                <div class="next-jt"><a href="'.$data['next']['link'].'">'.b2_get_icon('b2-arrow-right-s-line').'</a></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    public function shop_normal_action(){
        ...
        <div class="mg-t shop-single-action">
            <div class="shop-single-action-left" v-if="data !== '' && !data[<?php echo $post_id; ?>]['out_link']" v-cloak>
                <a class="button" target="_blank" :href="'<?php echo b2_get_custom_page_url('carts').'?id='.$post_id.'&count='; ?>'+count+'&index='+pickedMultiId" v-if="data !== '' && data[<?php echo $post_id; ?>].can_buy.allow && data[<?php echo $post_id; ?>].limit.can_buy"><?php echo __('购买','b2'); ?></a>
                <button disabled="true" v-else-if="data !== ''" v-text="data[<?php echo $post_id; ?>].can_buy.text" style="margin-right:10px" v-cloak></button>
                <button class="empty" @click="addCart" v-if="!inCart()" :disabled="data !== '' && (!data[<?php echo $post_id; ?>].can_buy.allow || !data[<?php echo $post_id; ?>].limit.can_buy)">
        ...
        <div class="shop-single-top" ref="shopSingle" data-id="<?php echo $post_id; ?>">
            ...
            <div class="shop-single-data">
                ...
                <div class="shop-title-box">
                    ...
                    <div v-if="data && data[id].limit.limit" class="red"><?php echo sprintf(__('限购商品：每个账户%s限购%s件','b2'),'<span v-if="data[id].limit.time">'.sprintf(__('每%s天','b2'),'<b v-text="data[id].limit.time"></b>').'</span>','<b v-text="data[id].limit.limit"></b>'); ?></div>
                </div>
                <?php 
                if($type){
