...
class Normal{
    ...
    public function normal_options_page(){
        ...
        //网站连接符
        $normal->add_field(array(
            'name'    => __( '标题连接符前后是否去掉空格', 'b2' ),
            'desc'    => __( '一些朋友的原始数据中是没有连接符的，此功能为了保留原始的标题格式，以免被SEO降权。', 'b2' ),
            'id'=>'separator_space',
            'type'=>'select',
            'options'=>[
                0=>__('保留空格','b2'),
            ...
            'default'=>self::$default_settings['separator_space']
        ));
        ...
        $login->add_field(array(
            'name'    => __( '邮箱注册白名单', 'b2' ),
            'desc'    => sprintf(__( '如果此白名单不为空，用户只允许白名单中的邮箱注册。每个邮箱占一行，例如%s', 'b2' ),'<p><code>@qq.com</code><br><code>@163.com</code></p>'),
            'id'=>'email_whitelist',
            'type'             => 'textarea_small',
            'default'          => self::get_default_settings('email_whitelist')
        ));
        $login->add_field(array(
            'name'    => __( '邮箱注册黑名单', 'b2' ),
            'desc'    => sprintf(__( '如果此黑名单不为空，黑名单中的邮箱地址将不允许注册。每个邮箱占一行，例如%s', 'b2' ),'<p><code>@qq.com</code><br><code>@163.com</code></p>'),
            'id'=>'email_blacklist',
            'type'             => 'textarea_small',
            'default'          => self::get_default_settings('email_blacklist')
        ));
        ...
        $login->add_field(array(
            ...
            'desc'=>__('开启后，登录窗口会只显示微信二维码','b2'),
            'default'          => self::$default_settings['wx_show_qrcode'],
        ));
    ...
    public static function write_settings(){
        ...
        $write->add_field(array(
            ...
            'type'    => 'text',
            'desc'=>sprintf(__('请直接填写%sID','b2'),$collection_name,$collection_name)
        ));
        ...
        $write->add_field(array(
            ...
            'default'=>0,
            'desc'=>__('允许发布下载后，用户可以在前端投稿中发布下载资源','b2'),
        ));
    ...
    public static function pay_settings(){
        ...
        $pay->add_field(array(
            ...
            'id'      => 'wecatpay_secret',
            'type'    => 'text',
            'desc'=>__('进入微信商户平台->API安全->API密钥(v2)','b2'),
            'after_row'=>'</div>'
        ));
    }
    ...
    public static function weixin_settings(){
        ...
        $weixin->add_field(array(
            ...
            'before_row'=>'
            <hr><h2>是否开启微信自动回复</h2>
            <p>用户在公众号中输入内容，会自动回复相关内容，如果您不需要，可以关闭</p>
            '
        ) );
    ...
    public static function mail_message(){
        $mail = new_cmb2_box(array(
            'id'           => 'b2_mail_message_options_page',
            'tab_title'    => __('邮件消息开关','b2'), 
            'object_types' => array( 'options-page' ),
            'option_key'      => 'b2_normal_mail_message',
            'parent_slug'     => 'admin.php?page=b2_normal_main',
            'tab_group'    => 'b2_normal_options',
        ));

        $mail->add_field( array(
            'before_row'=>'<p>'.__('使用此功能的前提是，wp中安装了邮件发送插件，安装方法请自行百度“wordpress邮件发送插件”，频繁发送邮件，可能会被收件服务器列为垃圾邮件，所以发送频率过高的邮件，建议选择关闭','b2').'</p>',
            'name'    => __('管理员收件邮箱','b2'),
            'id'      => 'msg_admin_email',
            'type'    => 'text',
            'default' => '',
            'desc'=>__('请填写管理员的邮箱，用于收取发给管理员的通知邮件','b2')
        ) );

        $mail->add_field( array(
            'name'    => __('有待审的文章、帖子、快讯、问答、供求、导肮等通知管理员','b2'),
            'id'      => 'msg_pending_admin',
            'type'    => 'radio_inline',
            'options' => array(
                0 => __( '关闭', 'b2' ),
                1   => __( '开启', 'b2' ),
        ...
        $mail->add_field( array(
            'name'    => __('文章、帖子、快讯、问答、供求、导航等审核成功通知作者','b2'),
            'id'      => 'msg_pending_author',
            'type'    => 'radio_inline',
            'options' => array(
                0 => __( '关闭', 'b2' ),
                1   => __( '开启', 'b2' ),
        ...
        $mail->add_field( array(
            'name'    => __('用户置顶供求给管理员通知','b2'),
            'id'      => 'msg_infomation_sticky',
            'type'    => 'radio_inline',
            'options' => array(
                0 => __( '关闭', 'b2' ),
                1   => __( '开启', 'b2' ),
        ...
        $mail->add_field( array(
            'name'    => __('认证审核通过，通知申请者','b2'),
            'id'      => 'msg_verify_author',
            'type'    => 'radio_inline',
            'options' => array(
                0 => __( '关闭', 'b2' ),
                1   => __( '开启', 'b2' ),
        ...
        $mail->add_field( array(
            'name'    => __('圈子付费查看回答通知答主','b2'),
            'id'      => 'msg_circle_read_answer_pay',
            'type'    => 'radio_inline',
            'options' => array(
                0 => __( '关闭', 'b2' ),
                1   => __( '开启', 'b2' ),
        ...
        $mail->add_field( array(
            'name'    => __('圈子付费查看通知帖子作者','b2'),
            'id'      => 'msg_circle_hidden_content_pay',
            'type'    => 'radio_inline',
            'options' => array(
                0 => __( '关闭', 'b2' ),
                1   => __( '开启', 'b2' ),
